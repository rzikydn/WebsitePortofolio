...
function X({ className, style, ...props }) {
  return (
    <svg
...
import "./morphing-tabs.css";

const DRAG_THRESHOLD = 5;
const MAX_TAB_WIDTH = 260;
...
function liquidTabPath(tabLeft, surfaceWidth, tabWidth) {
      ...
      {active && surfaceHost && surfaceWidth > SURFACE_INSET * 2
        ? createPortal(
            <svg
              ...
              className={cn(
                "morphing-tabs-surface-svg pointer-events-none absolute inset-x-0 top-0 h-[108px] w-full text-[#fafaf8]",
                dragging ? "z-20" : "z-0",
                surfaceClassName,
              )}
            >
...
function LiquidSurfacePath({ left, surfaceWidth, tabWidth }) {
...
export function MorphingTabs({
  ...
  return (
    <div
      ref={rootRef}
      className={cn(
        "morphing-tabs-root relative isolate min-w-0 overflow-hidden rounded-[2rem] bg-[#292929] text-white",
        classNames?.root,
        className,
      )}
    >
      <div className="morphing-tabs-rail-wrap relative h-20">
        <div
          ref={railRef}
          role="tablist"
          ...
          className={cn(
            "morphing-tabs-rail relative z-30 flex h-full gap-3 md:gap-4",
            classNames?.rail,
          )}
        >
          {orderedItems.map((item, index) => {
            ...
            return (
              <SpringTab
                ...
                className={cn(
                  "morphing-tab-spring group absolute left-0 top-0 flex touch-pan-y items-stretch",
                  TOUCH_GESTURE_CLASS,
              ...
              >
                <div
                  ...
                  className="morphing-tab-inner relative flex items-stretch"
                >
                  {!isActive ? (
                    <span
                      aria-hidden
                      className={cn(
                        "morphing-tab-hover-bg absolute inset-x-0 bottom-2 top-0 rounded-[1.25rem] transition-colors duration-200",
                        isDragging
                          ? "bg-[#3a3a3a]"
                  ...
                  <button
                    ...
                    className={cn(
                      "morphing-tab-btn group relative z-10 flex h-full w-full min-w-0 items-center gap-2 rounded-t-[1.5rem] pl-3 text-left outline-none transition-colors",
                      isActive
                        ? "is-active text-[#181818]"
                  ...
                  >
                    ...
                    <span
                      className={cn(
                        "morphing-tab-label min-w-0 truncate whitespace-nowrap text-base font-medium tracking-[-0.025em]",
                        classNames?.label,
                      )}
                    >
                  ...
                  {onClose ? (
                    <button
                      ...
                      className={cn(
                        "morphing-tab-close-btn absolute right-2 top-1/2 z-20 grid size-6 -translate-y-1/2 place-items-center rounded-full text-[#9aa0a8] transition-colors hover:bg-black/[0.06] hover:text-[#4b5563] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-current",
                        !isActive &&
                          "inactive top-[calc(50%-4px)] text-white/45 hover:bg-white/[0.08] hover:text-white/80",
                        classNames?.close,
                      )}
                    >
                      <X aria-hidden className="size-3.5 stroke-[1.5]" style={{ width: 14, height: 14 }} />
                    </button>
                  ) : null}
                </div>
              </SpringTab>
      ...
      <div
        ...
        className={cn(
          "morphing-tabs-panel relative z-20 mx-4 min-h-64 overflow-hidden rounded-[1.75rem] bg-[#fafaf8] text-[#181818]",
          classNames?.content,
        )}
      >
